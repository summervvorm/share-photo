...
import static com.example.photos.constant.RedisConstant.POP_PIC;
import static com.example.photos.constant.RedisConstant.POP_RECOMMEND_EXPIRE_TIME;
...
public class PopRecommendStrategyImpl implements RecommendStrategy {
    ...
    public List<PictureInfoDTO> getRecommendPic(Integer userId) {
        ...
        List<PictureInfoDTO> pictureInfoDTOS = (List<PictureInfoDTO>) redisService.get(POP_PIC);
        if (null == pictureInfoDTOS || CollUtil.isEmpty(pictureInfoDTOS)) {
            ...
            redisService.set(POP_PIC,pictureInfoDTOS,POP_RECOMMEND_EXPIRE_TIME);
