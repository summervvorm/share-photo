    ...
    <div class="nav-bar">
      ...
      <div class="right-menu">
        ...
        <el-dropdown @command="handleCommand">
          <el-avatar :size="40" :src="this.$store.state.currentUser.header" />
    ...
    <div class="tabs-view-container">
      <div class="tabs-wrapper">
        <span :class="isActive(item)" v-for="item of this.$store.state.tabList" :key="item.path" @click="goTo(item)">
          {{ item.name }}
          <i class="el-icon-close" v-if="item.path != '/Index'" @click.[]="removeTab(item)" />
        </span>
      </div>
...
export default {
  ...
  methods:{
    ...
    handleCommand(command) {
      ...
      if (command == 'logout') {
        api.post('/api/users/logout').then(({ data }) => {
