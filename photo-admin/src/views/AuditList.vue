    ...
    <div class="table">
      <el-table
        ...
        <el-table-column
          label="创建日期"
          width="170">
          <template slot-scope="scope"><i class="fa-regular fa-clock"></i> {{ scope.row.picCreatTime.replace("T"," ") }}</template>
        </el-table-column>
        <el-table-column
          label="图片名"
        >
          <template slot-scope="scope">{{ scope.row.picName.replace("http://arrows.oss-cn-beijing.aliyuncs.com/photos/photos/","")}}</template>
        </el-table-column>
        <el-table-column
          label="图片"
          width="70"
        ...
        >
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="pass(scope.$index,scope.row)"
              type="primary"
              size="small">
              通过
            </el-button>
...
export default {
  ...
  methods: {
    allIgnore(){
      ...
      api.delete("/api/picture/auditPics",{
        data:data1
      }).then(function (res){
        console.log(res)
      }).catch(function (err){
    ...
    allPass(){
      ...
      api.post("/api/picture/auditPics",data).then(function (res){
        console.log(res)
      }).catch(function (err){
        console.log(err)
      })
    ...
    pass(index, rows) {
      ...
      for(let i=0;i<data[0].color.length;i++){
        colors=colors+data[0].color[i]
      }
      data[0].color=colors
      const vm=this
      api.post("/api/picture/auditPics",data).then(function (res){
        console.log(res)
        // vm.tableData[index].splice(index, 1);
        vm.getPicList()
      }).catch(function (err){
        console.log(err)
      })
